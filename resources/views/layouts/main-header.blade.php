					...
					<div class="main-header-right">
						<ul class="nav">
                            ...
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                ...
                                <a rel="alternate" hreflang="{{ $localeCode }}" href="{{ LaravelLocalization::getLocalizedURL($localeCode, null, [], true) }}" class="mx-2" >
                                    {{ $properties['native'] }}
                                </a>
...
{{--                            @if(auth::user()->Status == "مفعل" && auth()->user()->unreadNotifications->count() > 0)--}}
                            @can('الاشعارات')
                                    ...
                                    <div class="dropdown-menu">
                                        <div class="menu-header-content bg-primary text-right">
                                            <div class="d-flex">
                                                ...
                                                <a href="{{route('MarkAsRead_all.index')}}">
                                            ...
                                            <p class="dropdown-title-text subtext mb-0 text-white op-6 pb-0 tx-12 ">عدد الفواتير الغير مقروءه
                                                <span id="notifications_count">{{ auth()->user()->unreadNotifications->count() }}</span>
                                            </p>
                                        </div>
                                        <div class="main-notification-list Notification-scroll" id="unReadNotifications_count">
                                            @foreach(auth()->user()->unreadNotifications as $notification)
                                                <a class="d-flex p-3 border-bottom" target="_blank" href="{{ LaravelLocalization::localizeUrl(route('section.Details',$notification->data['id'])) }}">
									...
									<div class="main-header-profile bg-primary p-3">
										<div class="d-flex wd-100p">
											...
											<div class="mr-3 my-auto">
												<h6>{{ trans('messages.hi') }} {{ Auth::user()->name }}</h6><span>{{ Auth::user()->email }}</span>
											</div>
										</div>
									</div>
									<a class="dropdown-item" href=""><i class="bx bx-user-circle capt"></i>{{ trans('messages.hi') }} {{ Auth::user()->name }}</a>
